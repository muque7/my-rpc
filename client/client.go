...
func (c *Client) Call(serviceName string, methodName string, fptr interface{}) {
	container := reflect.ValueOf(fptr).Elem()

	f := func(req []reflect.Value) []reflect.Value {
		cliTransport := transport.NewTransport(c.conn)
		...
		err := cliTransport.Send(codec.Data{ServiceName: serviceName, MethodName: methodName, Args: inArgs})
